...
it('Vote Routes - Successful', async done => {
  ...
  var response = await request.post('/api/routes/1/vote').set('x-auth-username', 'Mitul2').set('x-auth-token', token).send(payload)
...
it('Vote Routes - Route Not Found', async done => {
  ...
  var response = await request.post('/api/routes/fakefakefake/vote').set('x-auth-username', 'Mitul2').set('x-auth-token', token).send(payload)
