...
export default function routesReducer (state = initialState, action) {
  switch (action.type) {
    ...
    case 'UPDATE_RATING':
      const tempAllRoutes = [...state.allRoutes]
      const routeFound = tempAllRoutes.find(r => r.routeid === action.payload.routeid)
      if (routeFound) {
        routeFound.votes = [...action.payload.votes]
      }
      return { ...state, allRoutes: tempAllRoutes }
    case 'ADD_CREATED_ROUTE':
      const addCreatedRoutes = state.createdRoutes.slice()
      const addAllRoutes = state.allRoutes.slice()
    ...
    case 'REMOVE_CREATED_ROUTES':
      const removeCreatedRoutes = state.createdRoutes.filter(function (obj) {
        return obj.routeid !== action.payload
      })
      const removeAllRoutes = state.allRoutes.filter(function (obj) {
        return obj.routeid !== action.payload
      })
