...
class RoutePage extends Component {
  ...
  componentWillUnmount () {
    ...
    updateTraffic = (routeid, routeStart) => {
      const self = this
      const traffic = [...self.props.traffic]
      const trafficFound = traffic.find(t => t.routeid === routeid)
      if (trafficFound) {
        const index = traffic.indexOf(trafficFound)
        routeStart
    ...
    sortRoute = (sort) => {
      if (sort === 'TYPE') {
        const sortByTitle = this.state.Routes
      ...
      if (sort === 'RATING') {
        const sortByRating = this.state.Routes
      ...
      if (sort === 'DISTANCE') {
        const sortByDistance = this.state.Routes
...
function MapDetailCard (props) {
  ...
  // image selection.
  const image = props.route.routetype === 'Biking' ? bike : props.route.routetype === 'Walking' ? jog : undefined
  const ratingAvg = getAvgRating(props.route.votes)
